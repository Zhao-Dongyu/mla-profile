...
class DeepseekAttention(nn.Module):
    def __init__(self, hidden_size: int, num_attention_heads: int, q_lora_rank: int, qk_rope_head_dim: int, 
        ...
        self.wrapper = flashinfer.mla.BatchMLAPagedAttentionWrapper(
            workspace_buffer, backend= "fa3"#"auto"
        )
    ...
    def forward(self, hidden_states_q: torch.Tensor, q_position_ids: torch.LongTensor, compressed_kv: torch.Tensor):
        ...
        kv_indptr = torch.arange(0, bsz + 1).to(0).int() * kv_seq_len
        kv_indices = torch.arange(0, bsz * kv_seq_len).to(0).int()
        kv_lens = torch.full((bsz,), kv_seq_len, dtype=torch.int32).to(0)
        ...
        self.wrapper.plan(
            ...
            torch.bfloat16, # q_nope.dtype,
            torch.bfloat16, # compressed_kv.dtype,
        )

        q_nope = query_states[:, :, :, : self.kv_lora_rank].squeeze(2)
        q_pe = q_pe.squeeze(2)
        ckv = compressed_kv.transpose(0, 1)
        k_pe = k_pe.squeeze(0).transpose(0, 1)
        attn_output = self.wrapper.run(q_nope, q_pe, ckv, k_pe, return_lse=False)
        attn_output = torch.einsum('bhqc,hdc->bhqd', attn_output.unsqueeze(2), out_absorb)
